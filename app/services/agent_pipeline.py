...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "①「样本中位→基准价→最优报价」三步反算表（每步写数字+依据），报价单最终数字必须**等于**反算表结果；"
            "②**规模修正只允许下修、禁止上修**：与本项目范围明显不同的样本（含硬件供货/多场站/多期叠加）先剔除或"
            "按比例下修；**同规模最相近样本原值直接作基准价锚点，不乘任何上修系数**；最优报价=锚点×0.98 附近按扣分斜率"
            "微调后取整到万，**报价不得高于锚点**（报价远超同规模样本=成本加成回潮，判不过）；"
            "③成本合计只允许出现在「底线校验」行，**禁止 成本×系数=报价 的任何写法**；"
            "④成交价公开的同类项目（同规模最相近）报价作为基准价主锚点，行情库 get_history_price 中位辅助校验。"
            ...
            "②字体统一：正文中文**宋体**（模板有指定则按模板）、西文与数字 **Times New Roman**、正文小四（12pt）/"
            "表格五号（10.5pt）/标题黑体——**禁止引入 Calibri/等线/Consolas 等西文默认字体渲染中文**，"
            "每个 run 必须显式设置 w:rFonts（ascii/hAnsi/eastAsia 三处都设），同一文件禁止混用多种字号；"
            "验收时解包检查 rFonts 与 sz 的分布（出现 Calibri/等线或 3 种以上字号=判不过），"
        ...
        prompt = (
            ...
            "**方案深度量化纪律（防「能交但薄」，验收逐节核）**：技术专项响应文件按下列结构成文，缺哪节补哪节、"
            "短哪节扩哪节——总体方案：总体设计思路→数据采集体系总体架构（分层说明：接入层/协议转换层/数据治理层/"
            "应用层，每层一段实质描述+关键设备清单表）→多协议转换方案（适配范围/转换引擎实现细节/性能保障）→"
            "网络冗余方案（主备切换机制）；分项技术方案按「六段式」逐类对象成文"
            "（现状痛点→调试范围与接口清单→技术路线分步→关键参数与时延预算→风险与对策→验收标准），"
            "每类对象正文不少于 500 字，另配「调试作业卡」（作业步骤/安全措施/工时估算/验收记录）；"
            ...
            "**自绘图配额**：每类对象至少 1 张调试作业流程图；总体方案 2-3 张（架构图/数据流图）；"
            "另配时延预算分解图、主备切换时序图各 1 张——图由 Hermes 自绘（matplotlib/mermaid，"
            ...
            "②大纲条目**不少于 600 条**（金标准目录 1278 条=业绩组数×证据页数+人员数×证件页数+页码索引——"
            "业绩每组一节、人员每人一节、资质每项一节，各组各人各证都要有自己的标题条目，不是合并成一段）；"
            ...
            "分项方案每类对象≥3,000×全部对象、实施组织与进度≥8,000、质量安全进度保障≥6,000、售后培训≥6,000、"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "扫描件页在文件里，不是只有文字叙述）；④报价反算硬验收门逐项核对：三步反算表（样本中位→基准价→最优报价）"
            "齐全、报价单数字=反算表结果、规模修正只下修不上修（报价=同规模锚点×0.98 取整且不高于锚点原值）、"
            "成本只出现在底线校验行（无成本加成写法）、"
            "报价依据三路（成本科目/行情样本/规则面）随件写明；⑤编号逐字 diff：信用代码/注册号/证书编号/采购编号"
            ...
            "（无竖线假表），rFonts 分布无 Calibri/等线/Consolas、正文字号统一（宋体小四/Times New Roman）。"
