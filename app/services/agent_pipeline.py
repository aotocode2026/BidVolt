...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "②**锚点纪律（防锚点作弊）**：第二步的基准价锚点必须是行情样本清单中**列出的具体样本原值**"
            "（写样本名+金额+source 编号），**禁止用「估计的有效报价均值 A2」或任何估算值充当锚点**；"
            "规模修正**只允许剔除或下修**样本，**严禁对锚点上修任何百分比**（出现「上修」字样=判不过）；"
            "最优报价=锚点×0.98 附近按扣分斜率微调后取整到万，**报价不得高于锚点原值**；"
            "若反算结果高于锚点=锚点选错，必须回退到同规模最相近样本原值重算；"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "齐全、报价单数字=反算表结果、**锚点=样本清单中的具体样本原值**（禁止估计 A2 充当锚点、禁止「上修」字样、"
            "报价=锚点×0.98 取整且不高于锚点原值）、"
            "成本只出现在底线校验行（无成本加成写法）、"
