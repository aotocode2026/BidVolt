...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "选取顺序：场景一致→口径一致（含硬件供货样本仅在本项目同样含硬件时可用）→形态一致；"
            "**多个样本形态相近时取金额较低者（取低不取高——换成高金额样本=变相加价，判不过）**；"
            "**锚点一经选定不得更换**：反算表与测算说明从头到尾只允许一个锚点；"
            "锚点写样本名+金额+source 编号，原值不乘任何系数；"
            ...
            "成本合计**不得高于同形态样本最高金额**——成本超过样本上限=科目虚高（重复计费/单价离谱），"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "齐全、报价单数字=反算表结果、**锚点=形态最接近样本原值**"
            "（多候选取低不取高、禁止换成高金额样本、禁止估计 A2 充当锚点、禁止「上修」字样、"
