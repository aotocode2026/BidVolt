...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "②**锚点选取纪律（防换样本作弊）**：锚点=行情样本清单中**形态与本项目最接近**的样本原值——"
            "选取顺序：场景一致（场站数据接入/接口调试平台类）→口径一致（含硬件供货样本仅在本项目同样含硬件时可用，"
            "本项目软硬件由应答人提供）→形态一致（多电站平台接入类优于单场站单接口类）。"
            "**形态最接近者原值直接作锚，金额高低不得作为替换理由**（换成高金额样本=变相加价，判不过）；"
            "**本项目锚点指定=「新能娄底望湄峰等电站接入智慧管控平台接口服务」70.00 万元**（2026-04，多电站接入平台，"
            "形态与本项目一致）——锚点=70.00 万原值（不乘任何系数），最优报价=70×0.98≈68.6 附近取整到万，"
            "**报价不得高于 70 万**；其他样本（348 万硬件多场站/192 万含硬件单场站等）只作校验参考，不得改锚。"
            "禁止用「估计的有效报价均值 A2」或任何估算值充当锚点；禁止「上修」字样（出现=判不过）；"
            "③**成本底线纪律**：成本科目按本项目真实构成逐项测算——人力=人数×驻场天数×人日单价"
            "（市场行情 1500-2500 元/人日，超出须注明依据），差旅/设备/管理费列实际口径；"
            "成本合计**不得高于同形态样本金额（本项目 70 万档）**——成本超过样本上限=科目虚高（重复计费/单价离谱），"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "齐全、报价单数字=反算表结果、**锚点=「新能娄底望湄峰等电站接入智慧管控平台接口服务」70.00 万原值**"
            "（禁止换成 192 万等含硬件高金额样本、禁止估计 A2 充当锚点、禁止「上修」字样、报价=70×0.98≈68 万附近"
            "取整且不高于 70 万）、"
