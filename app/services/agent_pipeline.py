...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "②报价必须 **≤ 规模修正后的行情样本上限**（与本项目范围明显不同的样本先按范围比例修正再取上限，"
            "不写修正理由则直接取样本最高值；报价远超样本上限=成本加成回潮，判不过）；"
            "③成本合计只允许出现在「底线校验」行，**禁止 成本×系数=报价 的任何写法**；"
            "④成交价公开的同类项目报价与行情库 get_history_price 中位作为基准价主锚点。"
            ...
            "**打包内容硬验收门（最终 zip 必须含全部 11 个条目，验收时 zipfile 列目录逐项核对，缺任一=判不过回修）**："
            ...
            "含税合价+合计行）与「报价依据说明」（采购编号/报价方式/最高限价有无/税率/报价构成/行情区间与样本清单/承诺——"
            "对外口径，不含内部成本明细）。"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "齐全、报价单数字=反算表结果、报价≤规模修正后的样本上限、成本只出现在底线校验行（无成本加成写法）、"
            "报价依据三路（成本科目/行情样本/规则面）随件写明；⑤编号逐字 diff：信用代码/注册号/证书编号/采购编号"
            "与来源原文并排比对无任何长度或字符差异；⑥打包内容硬验收门：最终 zip 逐项核对 11 个条目齐全"
            "（9 份正式文件+内部管理文件/报价测算工作簿.xlsx+内部管理文件/编制逻辑与评分响应记录.docx），"
            "报价单.xlsx 含「报价明细表」+「报价依据说明」两个 sheet。"
