...
async def run_agent_pipeline(session: AsyncSession, task: Task) -> None:
    ...
    if resume_session_id and not pre_chat:
        prompt = (
            ...
            "交付件必须携带证据——大卷（补充文件/专项响应文件）走**底稿骨架通道**成文："
            "先 slice_template_item 抽底稿骨架，正文/表格/证据图一律用 append_template_slice 挂到"
            "对应条目标题之下（image 节点两种来源：file_id+page 取企业资料库原件、path 取本地图片；"
            "服务端自动等比缩放居中、题注不进大纲）；只在底稿中确实定位不到条目时才用"
            "整文件直写通道，或改列『附件N：原件在库（扫描件随附）』清单；只写事实不留证据=不合格。"
            ...
            "目录条目数=业绩组数×证据页数+人员数×证件页数+页码索引，照此形态成文目录自然对齐。"
            "**证据装订可派专职子 agent（独立轮次预算）**：插图重活交给「证据装订」子 agent"
            "（任务书：评分矩阵+资产清单+目标章节+插图规范≤16×23.2cm 等比例不撑页；"
            "骨架通道：append_template_slice 的 image 节点（file_id+page 或 path）逐张插到对应条目下，"
            "写完 verify_template_slice → seal_template_item → package_response_zip；"
            "确需整文件加工时才用 download_project_material + python-docx + 覆盖上传 + render_qa_docx 复验）；"
            "证据=实际插入的图，「原件在库」清单只在硬阻塞时降级；验收时核对矩阵行与图数对齐，缺图判不过。"
            "**证据页数量化（照整本/整份装订口径）**：矩阵每行证据写明装订页数——审计/信用报告**整本装订**（三年审计 100+ 页一页不落），"
            ...
            "①三步反算表（A2 估计→基准价→最优报价，每步写数字+依据），报价单最终数字必须**等于**反算表结果；"
            "②**报价策略口径（客户决策，提问关获取）**：A2（有效投标人评标价均值）估计口径由客户在提问关选择——"
            "**限价锚定**（有最高限价时 A2 估计=限价原值）/ **样本锚定**（A2=同形态样本中位，客户可指定具体样本）/ "
            "**保守取低**（A2=最低同形态样本）；客户未回答默认**限价优先**（有限价锚限价，无限价锚样本中位）；"
            "测算说明必须**单行列明**：最高限价（如有）、A2 估计值、基准价 B=A2×0.98、最终报价——四行数字独立成行供服务端校验；"
            "禁止「上修」字样、禁止把 A2 换成无证据的任意估算值；"
            "最优报价=B 附近按扣分斜率（n2=0.5 缓坡在下方）取整到万，报价不高于 B、且 ≤ 限价、> 异常低价红线、> 成本底线；"
    ...
    while marker == MARK_COMPLETE and confirm_rounds < CONFIRM_ROUNDS and sid:
        ...
        confirm_prompt = (
            ...
            "扫描件页在文件里，不是只有文字叙述）；④报价反算硬验收门逐项核对：测算说明**单行列明**最高限价（如有）/"
            "A2 估计值/基准价 B=A2×0.98/最终报价四行数字，报价单数字=反算表结果、B=A2×0.98 算术成立、"
            "**A2 口径与客户提问关选择一致**（限价锚定/样本锚定/保守取低，未答默认限价优先——限价锚定时 A2=限价原值）"
            "（禁止「上修」字样、报价在 B 下方取整、≤限价、>异常低价红线）、"
            "成本按真实构成测算且不高于同形态样本金额（高于=科目虚高须削减）、成本只出现在底线校验行"
            "（无成本加成写法）、"
