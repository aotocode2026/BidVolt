...
def test_tables_inventory_rows_detail():
    """表格清册只描述：表头 + 逐行现状（行号+各列内容），填哪行由 agent 决定。"""
    from docx import Document

    from app.services.export_service import empty_table_rows, tables_inventory
    ...
    inv = tables_inventory(src.element)
    assert inv[0]["rows"] == 3 and inv[0]["cols"] == 3
    assert inv[0]["header"][:3] == ["表头A", "表头B", "表头C"]
    assert [r["row"] for r in inv[0]["rows_detail"]] == [0, 1, 2]
    assert inv[0]["rows_detail"][1]["cells"] == ["", "", ""]
