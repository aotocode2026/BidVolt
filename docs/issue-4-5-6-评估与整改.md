...
- 本地浏览器全流程 E2E（headless Chromium 模拟人操作）：**18/18 PASS**，覆盖注册登录→项目→上传解析（真实 LLM 抽取 3 条要求）→成果生成（技术标 1743 字/商务标 466 字实质正文断言，非占位）→校核→在线编辑→评标闭环→报价闭环→会话（真实 LLM）→真实 AnySearch 搜索→**知识检索**→**公告导入（SSRF 拒绝实测）**→**环境切换与连接测试**→终检导出→快照/任务恢复→刷新恢复；
- 服务器实测（部署后）：alembic 0017/0018 迁移完成，六进程 RUNNING，healthcheck OK；真实 LLM 技术标生成验证 **2126 字实质正文、逐条响应技术要求（10kV/30kA）、非占位草稿**（产品反馈回归在生产环境复验通过）；
