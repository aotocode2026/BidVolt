...
- **公网生产环境浏览器 E2E**（`--base http://47.100.182.3:28123`）：**25/25 PASS**（2026-08-17 Issue #12 轮，用产品同款真实招标 docx）——tender_parse 真实 LLM 抽取 **14 条**（task#120）、material_match（task#121）、bid_generate 技术标 **2387 字/商务标 1092 字**、无占位草稿、**无 Markdown 残留** + `quality.deliverables_ready=true`（task#122）、bid_review（task#123），ReviewRun/ScoreRecord 绑定成果版本；技术标正文页面渲染 2448 字（非 JSON）；
