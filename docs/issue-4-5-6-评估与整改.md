...
- 本地全量 pytest：`N passed / N skipped`（SQLite，含上述全部新用例）；
- 本地浏览器全流程 E2E（headless Chromium 模拟人操作）：覆盖注册登录→项目→上传解析（真实 LLM）→成果生成（技术标/商务标实质正文断言）→校核→在线编辑→评标闭环→报价闭环→会话（真实 LLM）→真实 AnySearch 搜索→**知识检索**→**公告导入**→终检导出→快照/任务恢复→环境切换与连接测试，`M/M PASS`；
