...
> 基于《后端架构设计文档》v0.1，对 4.1~4.10 各模块逐一细化
> 每个模块输出：接口契约（REST API）、数据表 DDL、核心流程时序、依赖与风险
> 技术栈假设：后端 **Python 3.11+ / FastAPI / PostgreSQL 15+ / Redis / S3 兼容对象存储**（与 Hermes 及解析 SDK 生态一致）。若团队技术栈不同，接口契约与 DDL 不变，仅实现层替换。
...
- 文件归属：`target=enterprise` 存入企业资料库；`target=project` 存入当前项目（可勾选"同时存入企业资料库"）
...
- **Profile = 企业**：每企业一个 Hermes profile，隔离 MEMORY 与技能沉淀，防止跨企业数据污染
- **Session = 项目**：每项目一个 session，隔离对话上下文
- 后端任务编排维护映射：`(enterprise_id, project_id) → (profile, session)`

### 4.4.3 任务编排接口与表

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | /projects/{project_id}/tasks | 提交任务：`{task_type, payload}`，task_type ∈ `tender_parse / material_match / bid_generate / bid_review / mock_evaluate / targeted_edit / chat` |
| GET | /tasks/{task_id} | 任务状态与结果 |
| GET | /tasks/{task_id}/stream | SSE 消息流（原样透传 Hermes 消息） |
| POST | /projects/{project_id}/tasks/{task_id}/interrupt | 补充材料后的插队重规划（可选） |
| POST | /projects/{project_id}/chat | 底部对话：`{message, selection?}`，返回流式消息 |
...
CREATE TABLE task (
    ...
    status          SMALLINT NOT NULL DEFAULT 1,   -- 1 排队 2 执行中 3 完成 4 失败 5 已取消
    payload         JSONB NOT NULL,
    result          JSONB,                          -- 完成结果（含文档写入请求）
    progress        JSONB,                          -- {plan:[],done:[],failed:[],percent}
...
sequenceDiagram
    ...
    U->>API: POST /projects/{id}/tasks {bid_generate}
    API->>API: 建 task(status=排队) + 租户上下文
    API->>Q: 入队
    Q->>H: 消费 → 以(enterprise profile, project session)执行
    H->>M: MCP 查企业资料/招标要求
    H-->>API: 消息回调（"正在生成技术标…"）
    API-->>U: SSE /tasks/{id}/stream 透传
    H->>M: MCP 写入成果（结构化内容 → 版本）
    H-->>Q: 完成
...
| 风险 1 | 长任务失败/进程重启 → task 有 retry_count 与断点恢复（Hermes checkpoints），超时上限按任务类型配置 |
| 风险 2 | Profile/Session 映射持久化 → 用 enterprise_id+project_id 作为稳定 key，重启后重建 session 引用旧会话 |
| 风险 3 | 模型成本失控 → 主推理用 qwen-max、轻量任务走 qwen-plus，LLM 调用计费接入统计面板 |
| 风险 4 | 插队重规划与执行中任务的一致性 → v1 实现为：执行中任务允许追加指令消息（Hermes 同 session 注入），不强制取消任务 |
...
- **确定性算法服务**：历史报价查询、单价测算、三类策略计算——纯 Python 函数模块（`quote/engine.py`），不依赖 LLM，可单元测试
- **AI 层**：Hermes 通过 MCP 调用算法服务取数；无评分公式时由 AI 生成建议价（区间+理由+风险）
- **UI 口径**：同时展示"测算价"与"AI 建议价"两个口径（ADR D10）

### 4.7.2 接口契约

| 方法 | 路径 | 说明 | 响应 |
|---|---|---|---|
| GET | /quotes/history | 历史中标查询：`?material_name=&material_code=&spec=&tenderer=&region=&year=&page=` | 分页列表 + `{sample_count}` |
| GET | /quotes/history/{id} | 物料详情：样本数、最低/最高/中位数/均价/最近中标价、价格趋势、相似样本 | 详情对象 |
| POST | /quotes/calculate | 单价测算：`{material_ref, cost, min_profit_rate, adjustments?}` | `{median, avg_1y, avg_region, avg_spec, adj_time, adj_region, adj_spec, cost, min_profit, cap, score_formula, suggested}` |
| POST | /quotes/strategies | 三类策略计算：`{calc_id, strategy: win/balance/profit}` | `{suggested_price, score, gross_margin, risk_level, basis}` |
| POST | /quotes/apply | 用户确认应用：`{strategy_result_id, note?}` | 201 `{new_version_no}` + 审计记录 |
| POST | /quotes/ai-suggest | AI 建议价（无公式场景）：`{calc_id 或成本信息, instruction}` | `{price_range, recommended, reasons[], risk_level, is_ai_suggest: true}` |

关键规则：
- `/quotes/apply` 是唯一写入门户：校验用户确认标志 → 写入报价单（新版本 version_type=5 报价应用）→ 写 AuditLog
- `/quotes/ai-suggest` 返回必须带 `is_ai_suggest: true` 与理由，UI 与"测算价"区分展示
...
CREATE INDEX idx_hp_query ON history_price(material_name, material_code, region, win_date);
...
CREATE TABLE quote_calc (
    ...
    result          JSONB NOT NULL,          -- 测算结果（见接口）
    strategy_results JSONB,                  -- {win:{}, balance:{}, profit:{}}
    ai_suggest      JSONB,                   -- 无公式场景 AI 建议 {price_range, reasons, risk}
...
- 全部为纯函数（输入确定 → 输出确定），单元测试覆盖：公式正确性、边界（成本>上限、无历史数据）
...
- 规则 = 招标评分细则（4.3 requirement.score_rule，优先） + 系统内置评审规则库（补充检查）
- 内置规则类别：否决风险、文件完整性、资料有效性、跨文件一致性、方案完整性、内容针对性、报价合理性、格式规范
- 执行：内置规则为确定性检查（代码实现）+ 部分语义检查（Hermes）；评分细则以确定性公式计算为主

### 4.8.2 接口契约

| 方法 | 路径 | 说明 |
|---|---|---|
| POST | /projects/{project_id}/evaluate | 触发模拟评标（异步）：`{deliverable_version_nos?}` |
| GET | /projects/{project_id}/scores | 最新评分结果：综合/商务/技术/报价分、否决风险数、缺失材料数、可提升分 |
| GET | /projects/{project_id}/scores/{score_id} | 评分项明细（含证据定位） |
| GET | /rules | 内置规则库列表 |
| PUT | /rules/{rule_id} | 启用/禁用/调参（内部配置） |
...
CREATE TABLE eval_rule (
    id              BIGSERIAL PRIMARY KEY,
    rule_code       VARCHAR(50) UNIQUE NOT NULL,  -- builtin_reject_01 / ...
    name            VARCHAR(200) NOT NULL,
    category        VARCHAR(50) NOT NULL,   -- reject/integrity/validity/consistency/...
    severity        SMALLINT NOT NULL,      -- 1 必须 2 建议
    weight          NUMERIC(5,3),
    config          JSONB,                  -- 参数
...
CREATE TABLE score_item (
    ...
    improvable      NUMERIC(6,2),
    suggestion      TEXT,
    evidence        JSONB NOT NULL,         -- {requirement:[坐标], deliverable:[节点], material:[file_id], search:[url]}
...
sequenceDiagram
    ...
    participant H as Hermes(模拟评标 Skill)
    participant DB as PostgreSQL
    U->>API: POST /projects/{id}/evaluate
    API->>DB: 读 requirement.score_rule + 成果最新版本
    API->>API: 内置确定性规则逐条检查（完整性/一致性/格式/报价公式）
    API->>H: 语义规则检查（针对性/合理性）+ 按评分细则打分
    H-->>API: 评分项 + 建议 + 证据定位
    API->>DB: 写 score_record + score_item
    API-->>U: 评分结果（可定位到证据）
```
...
| 项 | 说明 |
|---|---|
| 依赖 | 4.3 requirement（评分细则）、4.5 成果版本、Hermes 模拟评标 Skill |
| 风险 1 | 评分公式千差万别 → 确定性部分按 requirement.score_rule 的 structured 参数驱动，未识别出公式的项走 Hermes 语义评估并标"参考" |
| 风险 2 | 证据定位要求高 → score_item.evidence 必须非空（缺证据的评分项不算入总分的可解释部分），不满足则提示 |
| 风险 3 | 评分结果与后续"提升建议闭环"联动 → 4.8 产出 score_item.suggestion 作为 4.9/后续建议模块的输入源 |
