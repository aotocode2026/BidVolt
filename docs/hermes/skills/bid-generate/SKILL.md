...
| `get_deliverable_content` | 读取已有成果（校核模式）或当前版本 |
| `save_deliverable` | 写新版本（version_type=AI生成/AI校核） |
...
2. 三份成果可**并行**执行（MCP `supports_parallel_tool_calls`）：
   ...
   - **报价单**：按 quote_rule 填报价，价格建议来自 `calculate_quote`；无评分公式时给出 AI 建议区间（标注 is_ai_suggest）
3. 内容来源规则（**P1 禁止编造**）：
...
5. `save_deliverable` 保存（带 change_note 说明改动）。
...
- 报价：有评分公式走 `calculate_quote` 确定值；无公式的 AI 建议必须标注"AI 建议（区间）"并附风险。
- 校核模式不得覆盖原始上传版本——`save_deliverable` 永远生成新版本。
