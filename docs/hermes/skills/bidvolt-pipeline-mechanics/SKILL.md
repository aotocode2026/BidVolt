...
- **统一修复链**（每个整文件直写产物上传前跑一遍）：①**只删悬空**的 headerReference/footerReference（按 `word/_rels/document.xml.rels` + zip 成员判定：目标部件不存在才删；部件在的保留，并在删内嵌 sectPr 之前**上提到 body sectPr**——全清=毁页码）；②删段落 pPr 内嵌 sectPr；③删 `w:br type=page` 与 pageBreakBefore；④删无文本无图空段；⑤表尾/文末空段修剪。脚本：`scripts/fix_docx_render.py <docx...>`（回执 `hf_ref_dropped`/`hf_ref_kept`/`hf_ref_restored` 三个计数可审计）；跑完本地 `libreoffice --headless -env:UserInstallation=file:///tmp/lo_prof_<n> --convert-to pdf --outdir /tmp/lo_test <f>.docx` 验一把，再 PUT 上传+render-qa 复验。
- **大卷骨架成文配方（技术专项响应文件 / 商务补充文件，issue #67）**：这两个"大卷"也必须走**底稿骨架通道**，不再用 `Document()` 从零重建——重建会丢模板条目骨架（217 实测：补充文件的二级子项「1.1 汇款凭证 / 1.2 响应保证金银行保函格式 / 1.3 响应保证金明细表 / 1.4 银行基本账户证明」一个独立标题都没有，全被并进一句正文）。工序：①`slice_template_item(file_id=底稿, req_id=该条目)` 抽骨架（模板条目区间字节级复制）；②逐条目标题 `append_template_slice(nodes=[...], heading=该节标题, page_break=false)` 写正文/表格；③证据扫描件用 **image 节点**挂到对应条目之下——`{"type":"image","file_id":<企业资料库原件 file_id>,"page":<PDF 页码>,"width_cm":15,"caption":"图：…"}` 或 `{"type":"image","path":"/tmp/xxx.jpg","width_cm":15,"caption":"图：…"}`（服务端等比缩放居中、宽 ≤16cm/高 ≤23.2cm、题注楷体 10.5 居中且**不写大纲级别**；单张 ≤12MB，超了先压缩）；④`verify_template_slice`（原文逐字 ⊂ 底稿）；⑤`seal_template_item`；⑥`package_response_zip`。**纪律**：正文只能挂在对应条目标题之下，不得删除/改写模板条目行；证据图缺失=判不过；`upload_deliverable_file` 只用于底稿中确实定位不到的条目或 xlsx/pdf。打包回执带 `skeleton_scan` 信号（对照底稿顶层条目名报缺失）供验收核对——交付件常用"章节号"体系、与底稿条目标号不同体系，故该信号只作提示、不做硬判。
- **媒体压缩必须同步 ContentType（217 图片不显示事故）**：把图片重编码（如 PNG→JPEG）时，媒体部件名/`[Content_Types].xml` 必须与实际字节格式一致——**无扩展名部件（`word/media/imageN.`）不能被 `\.(png|jpe?g)$` 这类按扩展名重命名的正则覆盖**，否则 Word 按声明类型解码失败、图片无法显示。压缩用 `scripts/compress_docx_media.py`（重编码后按输出格式改写 Override）；打包/上传前用 `scripts/repair_docx_media_types.py` 校验修复（按魔数探测，只改 ContentType，不动文件名与 rels）。验收：解包后“声明 png 的实际字节必为 PNG 魔数、声明 jpeg 的必为 JPEG 魔数”，错配数必须为 0。
- **压缩/重命名媒体后必须清掉旧 Override（217 Word 报"文件已损坏"事故，issue #69）**：媒体部件从 `imageN.` 重命名成 `imageN.jpeg` 时，rels 与 `Default Extension` 都改了，**但按旧部件名写的 `Override`（`/word/media/imageN.`）留在 `[Content_Types].xml` 里** → 内容类型表指向不存在的部件，Word 判"包结构不一致"直接拒绝打开（LibreOffice/python-docx 只查实际存在的部件，所以平台渲染质检全绿、问题只在 Word 侧暴露，整整潜伏了 3 个月）。排查一行命令：解包后比对 `[Content_Types].xml` 的 Override PartName 与 zip 实际条目，差集非空即中招（217 实测 402 条）。修复：删掉悬空 Override（`scripts/repair_docx_media_types.py` 已内置），平台侧 `docx_normalize` 也会在四个产物入口自动清理；验收：差集为空 + Word 能打开。
...
- **裁剪底稿成文配方（202 实测，四轮拒载/反排/identity 失败后收敛）**：整文件直写必须**裁剪底稿为仅条目内容**（文件开头第一行=条目标题，否则打包 audit `identity_issues`「正文开头不含自身条目标题」+ 模板合规问题「专项响应文件 docx 里混入整个采购文件」）。裁剪三段式：①元素 deepcopy 后**插到 sectPr 之前**（lxml append 会加到 sectPr 后 → 之后 python-docx add_paragraph 全插到 sectPr 前 → 新内容整体反排到 base 内容前，症状=文件开头直接是正文第一章，字数却对得上，迷惑性强）；②**复制底稿 styles/numbering/fontTable/settings/theme 等 part**（新 Document() 默认样式表缺底稿自定义样式 pStyle 引用 → LibreOffice 拒载）；③clear_revisions：只清**悬空**的 sectPr 内 header/footerReference（悬空 rId 才是拒载根因，按 rels+zip 成员判定；目标部件存在的引用保留并在内嵌 sectPr 删除前上提到 body sectPr——全清=丢页码）+ 解包 w:ins + settings 关 trackChanges。配套坑：商务卷 >10MB 压缩时 rels 的 Target 替换只替换 Target 段（Id 与 Target 间有 Type 属性，整串 replace 不命中 → python-docx 打开 KeyError）；fill 全角空格留白会被 seal 重标【待补充：X】，日期留白用「　年　月　日」形态；`package_response_zip` 自动附「会话记录/」2 个 md（内部留档），对外提交版需本地重写 zip 剔除（zip 不能经 upload_deliverable_file 上传，仅 docx/xlsx/pdf）。完整配方+评审 P0 类红线（内部测算口径不得成文/正文定性不得与随附审计冲突/已签署扫描件优先于空白模板）见 `references/本项目-draft-extract.md`；LibreOffice 拒载二分定位脚本 `scripts/bisect_render.py`。
...
- `scripts/fix_docx_render.py` — 整文件直写 docx 渲染修复链（**只删悬空** headerReference/footerReference + 有效引用上提保留 / 内嵌 sectPr / 分页符 / 空段），用法见第 6 节。
