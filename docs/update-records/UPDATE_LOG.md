...
<a id="2026-09-15-0400-fix-tier-check-patches"></a>

## 2026-09-15 04:00 · fix · 档位自检补丁：保守方向 + 否定阈值 + 扣分型条款保护

| 字段 | 值 |
|---|---|
| id | 2026-09-15-0400-fix-tier-check-patches |
| datetime | 2026-09-15T04:00:00+08:00 |
| type | fix |
| status | released |
| scope | review, scoring |
| related | issue #71, issue #70, discussion #62 |

### 为什么做这次更新

项目 217 的前两次完整重评（run 256 / run 257）暴露档位自检的三处缺陷：

1. **可能抬高得分**：质量奖条目的规则档位是"获奖 +2 分"，模型选了该档但实际未获奖（got=0），
   自检按档位把 0 改成 2（虽然 `verdict=unsatisfied` 最终仍按 0 分落库，但 `tier_check` 记账误导，
   换一条 `verdict=partial` 的规则就会真的虚增）；
2. **漏纠门槛型档位**：高职称人数规则是 ≥30人/≥15人/<15人，要素引用明确写"不足 15 人"，
   但按"15人"做子串匹配会同时命中 ≥15 与 <15 两档 → 判为歧义 → 重问仍未收敛时保留了 3/3；
3. **扣分型条款被误当得分档**："无失信不扣分／罚款≥30万扣10分""查实打招呼请托扣20分"这类
   规则的档位是**扣分值**（-10/-20），被自检写成 `to=-10/-20`（落库分值未受影响，因为
   `verdict=satisfied` 时会按 full=0 归零，但记账与潜在路径都不对）。

### 具体做了什么

- `apply_tier_consistency` 加**保守方向**约束：档位分值高于当前得分时不改分，改为要求重问；
- 新增 `_tier_supported_by_evidence`：有方向词（≥/≤/</不少于/不足…）时**只认极性匹配**，
  无方向词才回退到档位名/条件文本/带单位数值 token —— 这样"不足15人"正确落到 <15 档；
- 新增 `resolve_tier_conflict`：重问仍未收敛时，按证据支持的档位**保守定分**（绝不低于当前档的证据）；
- 档位区间上限 ≤ 0（扣分型条款）时**直接跳过自检**。

### 影响范围

- 评分更保守、不再出现"自检把分数改高"的路径；`tier_check` 记账与实际得分一致；
- 结果：run 257 里 7 条触发自检——项目负责人 12→7、高职称人数 3→1、综合实力 30→28、
  质量奖不再虚增（保持 0）、扣分型条款不再被改写。

### 迁移 / 破坏性变更

- 无数据库迁移；无接口变更（`tier_check` 语义更严格）。

### 验证方式

- 单测：新增 3 例（不抬高得分 / 否定阈值定档 / 扣分型跳过），`test_review_evidence.py` 17 例全绿；
  `tests/module` 全量 **267 passed**（4 项为既有失败）；
- 生产：项目 217 连续两次完整重评（run 256 修复前 / run 257 修复后）对比，
  自检触发条目与结果见 issue #71 评论。

### 回滚方式

回退本次提交并重启 app/worker（评分数据保留）。
...
| 字段 | 值 |
|---|---|
| id | 2026-09-14-2300-feat-tier-consistency-and-checklist-coverage |
| datetime | 2026-09-14T23:00:00+08:00 |
| type | feat |
| status | released |
| scope | review, scoring |
| related | issue #71, issue #70, discussion #62 |
...
issue #70 把评分输入覆盖率修好（"证据不足" 11 → 2 条）后抽查发现：**检索与证据已经到位，
但"证据 → 档位"的换算出错，且方向都是多给分**——

- 「最近一年度采购人评价」规则写"未参加绩效评价得 **4** 分"，要素引用也写"未参加…按 4 分口径申报"，
  模型却给了 **5/5**；
- 「高职称+高级技师人数」规则写明 "<15 人得 **1** 分"，要素引用写"不足 15 人"，模型却给了 **3/3**；

另外两条规则（"商务响应完整性 30 分"、"质量奖 2 分"）**要素清单没生成**（批量漏掉、逐条重试也没补上）；
"质量奖"标书已如实声明未获得，语义上应判 0 分而非"证据不足"。

### 具体做了什么

- **档位表 + 三道自检**（`review_evidence.apply_tier_consistency`）：
  生成要素清单的同一批调用里按原文抄出档位表 `tiers=[{label,condition,min,max}]`；
  评分提示词带上档位表并要求输出 `selected_tier`；评分后自检——
  ①**证据优先**：要素引用只指向唯一档位时以该档为准（抓第 1 类错误）；
  ②**档位校正**：固定分值档位校正到该分值、区间档位越界夹回；
  ③**门槛型冲突**：所选档位含 ≥/≤/不少于/N人 等门槛但要素核验存在 `miss` → 判档位可疑，
  按档位表**重问一次**再定分（抓第 2 类错误）。
- **清单覆盖率硬检查**（`review_evidence.build_plan`）：分批（每批 5 条）+ 逐条两轮补齐 +
  **规则原文兜底要素**（`synthetic=true`），保证覆盖率恒为 100%；
  覆盖率落 `score.detail.checklist_coverage`。
- 档位表与自检结论落 `item.evidence.tiers / tier_check`（含 `applied / from / to / reason / need_reask`）。

### 影响范围

- 评分结果更保守、更可核对：多给分会按档位表纠正或重问；
- `GET /scores/{score_id}/items` 的 `evidence` 新增 `tiers` 与 `tier_check`；
  `GET /scores` 的 `detail` 新增 `checklist_coverage`；
- 单次评审的 LLM 调用略增（仅"档位与证据矛盾"的规则会多问一次，实测 4 条里 1 条触发）。

### 迁移 / 破坏性变更

- 无数据库迁移；无破坏性变更（新增字段 + 评分口径收紧，历史评分记录保留）。

### 验证方式

- 单测：`apply_tier_consistency` 三分支（证据优先 / 区间夹取 / 门槛冲突重问 / 无档位跳过）、
  `build_plan` 覆盖率与兜底要素、档位表解析；`tests/module` 全量 **259 passed**（4 项为既有失败）；
- 生产定点复跑 4 条规则（真实模型）：
  | 规则 | 修前 | 修后 |
  |---|---|---|
  | 最近一年度采购人评价（5 分） | 5.0 | **4.0**（档位"未参加绩效评价"） |
  | 高职称+高级技师人数（3 分） | 3.0 | **1.0**（档位"<15人"，触发重问） |
  | 质量奖（2 分） | 证据不足 | **0 分（unsatisfied，标书已声明未获得）** |
  | 商务响应完整性（30 分） | 证据不足 | **26.0（一般档）**，档位自检把 29 校正到区间内 |
  覆盖率：`{rules:4, elements:21, tiers:12, synthetic:1}` —— 含兜底也 100% 覆盖。

### 回滚方式

回退本次提交并重启 app/worker；已产生的评分记录保留，可重跑得到旧口径结果。

<a id="2026-09-14-1800-feat-review-evidence-scope"></a>

## 2026-09-14 18:00 · feat · 评分输入改为「块索引 + 核验要素清单 + 定向取文」

| 字段 | 值 |
|---|---|
| id | 2026-09-14-1800-feat-review-evidence-scope |
| datetime | 2026-09-14T18:00:00+08:00 |
| type | feat |
| status | released |
| scope | review, scoring, deliverable |
| related | issue #70, discussion #62 |

### 为什么做这次更新

评分引擎的输入有硬上限——每份交付件截前 **12,000 字**、全部文件合计 **30,000 字**，且按 artifact
顺序拼接。项目 217 实测覆盖率：技术专项响应文件 156,695 字只读到 **7.7%**（截断在"3.4 数据需求"）、
补充文件 24.3%、编制逻辑记录 11.2%、**价格四件（测算工作簿/响应函/报价明细表/报价单）0 字**
（预算已耗尽）。

后果：run 251 的 25 条评分项里 **11 条判"证据不足"**，而逐条核对交付件原文发现——服务方案（30 分）、
工作进度及保证措施（13 分）、创新激励机制（1 分）、高职称+高级技师人数（3 分）、研发投入占比（2 分）、
商务文件完整性（24 分）等内容**都写在标书里**，只是没被读到（关键词首次出现位置普遍在 12,000 字之后：
进度计划 18,955、里程碑 55,810、创新激励 26,811、高级技师 28,862）。

### 具体做了什么

新增 `app/services/review_evidence.py`，把"读标书"按「规则 → 待核验要素清单 → 分块索引 → 定向精读」
重排：

1. **分块索引**：按标题层级把交付件切成块（无标题长文按 8,000 字窗口续切），保留标题路径；
   xlsx 按 sheet 切块；
2. **待核验要素清单**：一次 LLM 调用把全部评分规则拆成"可核验要素"
   （element / evidence / keys 检索线索 / criteria 判定口径），每条规则 2-8 个要素；
3. **定向取文**：按要素线索给块打分取相关块全文，预算 **正文 4 万字 + 目录 5 千字**；
   关键词**零命中**时才启用兜底（把目录交给 LLM 选块）——只对少数规则触发，控制成本；
4. **编制方自述接入**：从《编制逻辑与评分响应记录》抽"评分点→响应位置"索引行与
   "未闭环/风险提示"段落作为**索引与提示**注入，prompt 明确**严禁作为得分依据**
   （其"预计得分"栏不参与打分）；
5. **口径可见**：评审回执落 `checklist` / `element_results`（逐要素 hit/partial/miss + 原文引用）/
   `evidence_scope`（块数、字数、是否受限）/`chunks_provided`，经 `item.evidence` 回给前端——
   让"证据不足"能区分**没写**与**没读到**。

### 影响范围

- 评分输入从"按文件截断拼接"变为"按要素定向取文"，**重评后的分数会变化**（原本判"证据不足"的
  条目会给出真实档位得分）；
- `GET /projects/{id}/scores/{score_id}/items` 的 `evidence` 新增四个字段
  （`checklist` / `element_results` / `evidence_scope` / `chunks_provided`），原有字段不变；
- 单次评审的 LLM 调用增加 1 次（要素清单批量生成），输入量约为原来的 1.5 倍；
- 评分规则拆解、判定口径、`verdict` 枚举与"不编分"红线均不变。

### 迁移 / 破坏性变更

- 无数据库迁移；无破坏性变更（新增字段 + 输入构造方式变化）。

### 验证方式

- 新增 `tests/module/test_review_evidence.py` 9 例（分块/长章节续切/按线索取文/预算/零命中兜底信号/
  兜底按块号取文/目录预算/编制记录抽取与提示块/评审提示词装配）；
- `tests/module` 全量 **249 passed**（4 项失败为 HEAD 上既有失败）；
- 真实交付件本地验证：技术件切成 256 块后，按评分点线索可检索到
  「项目负责人简历表：陈千里」「六、实施组织与进度计划」「四、工作规划描述（总体设计与实施方案）」
  等此前读不到的章节；
- 生产重评项目 217（run 255 / score 260）：
  **"证据不足" 11 → 2 条**；技术 8 条全部可评（90/100）、商务 8/10 条可评（54.3/68）；
  此前读不到的大项均给出真实档位——服务方案 30/30、工作进度及保证措施 13/13、
  创新激励机制 1/1、专利 1/2、综合实力 28/33、商务文件完整性 20/24；
  总分 92.0 → **85.89**、提分空间 8.0 → **23.7**（旧分是"只评了 8 条"的分母算出来的，
  覆盖面扩大后分数更接近真实水平）。
- 首轮上线（run 254）曾出现回归：批量要素清单调用解析失败 → 清单为空 → 检索零命中 →
  "证据不足"反升到 14 条；已定位并修复（分批 + 逐条补齐 + 规则原文词兜底 + 空命中兜底取文）。
- **遗留（下一层问题，另行跟进）**：①档位判定偏差——采购人评价按规则"未参加得 4 分"却给了 5/5，
  高职称人数证据显示"不足 15 人"（应 1 分）却给了 3/3，方向都是多给分；
  ②个别规则的要素清单未生成（如"商务响应完整性 30 分"），需把清单覆盖率做成硬检查；
  ③"质量奖"条目标书已如实声明未获得，应判 0 分而非"证据不足"。

### 回滚方式

回退本次提交并重启 app/worker；已产生的评分记录保留，可重新触发一次评审得到旧口径结果。
...
- 匹配口径同时改为**词元级**（值前后不能再是字母/数字）：原先裸 `in` 子串匹配会把
  conflict 里的截断值当成交付件里完整编号的命中——实测解阻后又暴露 `1100111730005250777`
  （17 位截断）命中完整发票号 `11001117300052507773`（18 位）的误伤，已一并修掉；
...
- 新增 `tests/module/test_docx_normalize.py`（7 例：注入页脚 / 空心页脚接回 / 幂等 / pgNumType
  ...
  （测试夹具改为按生产形态先归一化再落库）。
- 服务器实测（项目 217 回修 10 份 item_docx，逐份覆盖上传新版本、旧版本自动归档）：
  页码覆盖 `false → true` 10/10；大纲噪声 948 由 **412 → 0**、949 由 **306 → 0**、945 由 1 → 0；
  LibreOffice 转 PDF 抽查页码连续且每页可见——技术偏差表 1 页（首页=1）、专项响应文件 455 页
  （首页 1 / 中间 228 / 末页 455）、补充文件 349 页（1 / 175 / 349）、编制逻辑与评分响应记录
  5 页（1 / 3 / 5），页数与回修前一致（未因加页脚而改版）。
- 项目 217 的响应文件包重新打包**未完成**：被既有「二次识别冲突编号」门禁拦下
  （资产库 2026-09-11 二次分类产生的 `numbers_conflict` 候选值与交付件中的统一社会信用代码
  `91110111318157964Q`、票号 `11001117300052507773` 撞车）。该门禁与本次改动无关，
  需要单独确认冲突值处置后再出包；单份 docx 的预览/下载已带页码。
