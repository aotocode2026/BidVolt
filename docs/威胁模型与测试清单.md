...
- [ ] 两个企业分别注册，A 企业 Token 访问 B 企业全部对象接口（文件/任务/Requirement/成果/评分/报价/引用/审计）均 403/404
- [ ] 项目对象跨项目访问（同企业不同 project）被拒
- [ ] 下载签名 URL 无权限返回 403；签名过期返回 401
- [ ] RLS 策略用例：绕过应用层直接 SQL 查询另一 enterprise_id 返回空集
- [ ] citation/search_source/review_material_link 等关联表逐项覆盖

### A-2 任务级授权上下文（P0-2）

- [ ] 企业资料导入任务可调用 classify/upsert 企业资料工具；其他任务调用返回 403
- [ ] 项目材料读取、成果写入、评审提交、网络搜索分别授权，互不越界
- [ ] 恶意材料内容（Prompt Injection 样本）不改变工具白名单与对象范围
- [ ] 任务结束/取消后授权上下文失效，MCP 调用返回 401/403
...
- [ ] Code Provider 以非 root 受限进程运行，无法访问数据库/宿主机业务目录；CPU/内存/时间 rlimit 生效
- [ ] 解析器/转换器/解压器以非 root 受限进程运行，输入目录只读挂载，无外部网络依赖
- [ ] （演进预留，不纳入 V1 验收）强隔离沙箱（无网络、只读根文件系统的独立沙箱容器）——需平台支持多容器

### A-11 审计与配额（P1）

- [ ] 文件读取/下载、MCP 工具调用、外部调用、模型版本、评分、报价、权限、管理员操作均有审计记录
- [ ] 每租户存储/并发/模型 token/搜索/导出配额生效，超限行为符合 4.11.1
- [ ] 备份恢复演练：RPO ≤ 24h、RTO ≤ 4h 实测通过；注销数据 90 天后物理删除

### A-12 评分审核闭环（D-J）

- [ ] 18 个可提升点 → 前端取得 18 条独立、可定位、可操作数据
- [ ] 单条/批量上传材料 → 解析 → 关联 → 确认 → 重审 → 更新实际得分全链路
- [ ] 未确认的材料关联或 AI 修改不改变成果版本和实际得分
- [ ] 刷新/重登/重进项目后，逐条状态、材料关联、确认记录、新旧得分可恢复
- [ ] 总分、分类汇总与逐条得分来自同一冻结输入与规则版本
